...
class SimpleSSO_AuthPlugin extends AuthPlugin {
	...
	private function __construct(){
		...
		if(!$sso_url){
			throw new Exception('$sso_url was not set.');
		}
    ...
    $msg.="\n";
    //wfDebugLog( 'simple-sso', $msg );
    wfDebug('SimpleSSO: '.$msg);
	}
	...
	public function userLoadFromSession( $user, &$result ) {
		...
		if(!session_id()){
			$this->log('No session found, forcing start...');
			self::startSession();
		}
